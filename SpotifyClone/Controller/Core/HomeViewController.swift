...
enum BrowseSectionType {
    ...
    case recommendedTracks(viewModels: [RecommndedTrackListCellViewModel]) // 3
}
...
class HomeViewController: UIViewController {
    ...
    private func fetchData() {
        ...
        var recomendations: RecomendationsResponse?
        ...
        // Featured Playlists,
        APICaller.shared.getFeaturesPlaylists { result in
            defer {
                group.leave()
            }
            switch result {
        ...
        APICaller.shared.getRecommendedGenres { result in
            ...
            group.notify(queue: .main) {
                guard let newAlbums = newReleases?.albums.items, let playlists = featuredPlaylist?.playlists.items, let tracks = recomendations?.tracks else {
                    return
                }
        ...
        private func configureModels(newAlbums: [Album],
            ...
            sections.append(.recommendedTracks(viewModels: tracks.compactMap({
                return RecommndedTrackListCellViewModel(name: $0.name, artistName: $0.album.artists.first?.name ?? "-",
