...
//  SpotifyClone
//
//  Created by Rudolf Amiryan on 24.12.23.
//
...
final class PlayerControlsView: UIView {
    ...
    private let playPauseButton: UIButton = {
        let button = UIButton()
        button.tintColor = .label
        let image = UIImage(systemName: "pause",withConfiguration: UIImage.SymbolConfiguration(pointSize: 44, weight: .regular))
        button.setImage(image, for: .normal)
        return button
    }()
    ...
    @objc func didSlideSlider(_ slider: UISlider) {
        ...
        delegate?.playerCotrolsView(self, didSlideSlider: value)
    }
    
    @objc private func didTapBack() {
        delegate?.playerCotrolsViewDidTapBackwardButton(self)
    }
    
    @objc private func didTapNext() {
        delegate?.playerCotrolsViewDidTapFrowardButton(self)
    }
    
    @objc private func didTapPlayPause() {
        self.isPlaying = !isPlaying
        delegate?.playerCotrolsViewDidTapPlayPauseButton(self)
        
        // Update icon
        let pause = UIImage(systemName: "pause",withConfiguration: UIImage.SymbolConfiguration(pointSize: 44, weight: .regular))
    ...
    override func layoutSubviews() {
        ...
        subtitleLabel.frame = CGRect(x: 0, y: nameLabel.buttom+10, width: width, height: 50)
        
        volumeSlider.frame = CGRect(x: 10, y: subtitleLabel.buttom+20, width: width-20, height: 44)
        
        let buttonSize: CGFloat = 60
        playPauseButton.frame = CGRect(x: (width-buttonSize)/2, y: volumeSlider.buttom+30, width: buttonSize, height: buttonSize)
